...
class Timer:
    ...
    def check_key(self, key):
        ...
        # 超過時間窗格 → 清空緩衝區
        if self.last_time is None or now - self.last_time > self.window:
            self.key_buffer.clear()

        self.last_time = now

        # ✅ 去除連續重複鍵
        if not self.key_buffer or self.key_buffer[-1] != key:
            self.key_buffer.append(key)

        # ✅ 比對滑動視窗是否符合目標鍵組
        if list(self.key_buffer) == self.keys:
            self.start_countdown()
            self.key_buffer.clear()
        elif list(self.key_buffer) == self.keys1:
            self.start_countdown()
            self.key_buffer.clear()
